              ...
              @foreach($users as $user)
                <tr>
                  ...
                  <td>
                    ...
                    {{-- NÚT RESET MẬT KHẨU (dùng chung route admin.user_delete với action=reset) --}}
                    <a href="javascript:void(0);" 
                       data-id="{{ $user->id }}" 
                       data-url="{{ route('admin.user_delete') }}"
                       class="btn btn-icon btn-sm btn-warning resetDialog tip" 
                       title="Reset mật khẩu về 12345678">
...
<script>
  ...
  $(document).ready(function(){
    ...
    // Tạo account mới (AJAX giữ nguyên)
    ...
    // ========= RESET MẬT KHẨU về 12345678 (dùng chung route admin.user_delete) =========
    $(".resetDialog").click(function() {
      ...
      Swal.fire({
        ...
        text: 'Đặt lại mật khẩu về mặc định 12345678?',
        ...
        preConfirm: () => {
          return fetch(url, {
            ...
            body: JSON.stringify({'user_id': user_id, 'action': 'reset'}),
          })
          .then(response => {
            if (!response.ok) {
