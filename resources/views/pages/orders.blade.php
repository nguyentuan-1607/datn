                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    @foreach($data['orders'] as $key => $order)
                      @php
                        // Gộp tính toán bằng collection (đã eager load order_details để tránh N+1)
                        $qty   = $order->order_details->sum('quantity');
                        $price = $order->order_details->sum(function($d){ return $d->price * $d->quantity; });
                      @endphp
                      <tr>
                        <td class="text-center">{{ $key + 1 }}</td>
                        <td class="text-center">
                          <a href="{{ route('order_page', ['id' => $order->id]) }}"
                             title="Chi tiết đơn hàng: {{ $order->order_code }}">
                            {{ $order->order_code }}
                          </a>
                        </td>
                        <td class="text-center">{{ optional($order->payment_method)->name ?? '—' }}</td>
                        <td class="text-center">{{ $qty }}</td>
                        <td class="text-center" style="color:#16A34A;">{{ number_format($price,0,',','.') }}₫</td>

                        {{-- Hiển thị badge theo map STATUS của Model --}}
  ...
  <script>
    $(function(){
      $("#slide-advertise").owlCarousel({
        items:2, autoplay:true, loop:true, margin:10, autoplayHoverPause:true,
        nav:true, dots:false,
